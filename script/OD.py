...
state.transform.position = spawns[0].position + 248.5*forward - 255 * right
...
for i in range(110):
    ...
    hit = sim.raycast(state.transform.position + px*right +
...
# np.savetxt('OD.csv', a.T, delimiter=',', fmt='%.2f')
